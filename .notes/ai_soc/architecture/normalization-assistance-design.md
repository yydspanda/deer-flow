...
Status: 已接入常规语义核对、类型化补充与 Memory 消费；外网 DEV 使用 apply。跨场景质量验收、按需检查降噪和内网实测仍开放。

Date: 2026-09-17（历史实验保留原始口径）
...
### 收尾验收：Adapter 字段归属与历史覆盖回归

- `1984510` 已修正 EDR 当前进程/祖先进程混用：当前路径明确时从该路径派生名称，
  不再取祖先 `services.exe`；SAM/SYSTEM 文件哈希归每条消息的目标文件观察。
  冻结真实输入加合成提议已验证两条检测可正确合并，未重跑语义模型。
- 旧210条语料 `192 -> 191` 已定位为六条 HIDS 的 `detail.*` 映射遗漏，被五条
  EDR 解析改善抵消了大半。接通已有类型化字段及 provenance 后覆盖197条、强特征126条。
  六条分别为1985655、1973457、1980407、1975890、1983975、1978266。
  这不是全量语料重建或32组全部验收；旧 Run、索引与审核 Memory 不自动迁移。
- 主研判v47两次、经验起草v10一次真实调用均完成，无额外重试/降级/fallback，但
  2488604主研判和经验起草仍弱化/复述错误差异描述。结构通过不等于解释质量通过。
  程序匹配边界保持有效，模型解释质量仍开放，后续需拆分机器来源事实与旧模型原话。

原始证据与可读总结：`backend/.deer-flow/soc-validation/closure-20260917/README.md`。
下面保留本日此前的身份稳定性实现说明；后文旧实验中的未完成项以本节最新状态为准。
...
| 部分 | 当前实际能力 | 尚未完成 |
|---|---|---|
| Adapter | message 优先、标准实体、原文和来源；KV v4 保留语法区间与 dotted key；EDR 当前进程/目标文件分开 | 不保证未知字段语义全部接住 |
| 模型补充 | 已进入正式 Runtime，不依赖验证脚本；原文、对象、检测、业务线索分别绑定来源 | 首次抽取不保证每个场景都完整正确；32 组尚未全部实测 |
| Runtime | v2 支持 objects/events/context/additional facts，合并、调用留痕和同输入事实复用；外网 DEV apply，部署默认 off | 失败仍保留原始事实继续研判；不能把“成功返回”算作语义准确率通过 |
| 按需检查 | 每个 Run 已保留输入、补充、合并及消费记录 | 原 maintenance 的噪音归并与按需汇总入口仍待做，不作为运营必办任务 |
| 指纹消费者 | apply 使用 Profile 9/v7，消费标准对象/检测身份；已验证相同输入重跑稳定和新增行为不被旧经验覆盖 | 更大质量集、首次跨告警稳定性和内网模型仍未完成验收 |
...
- 2026-09-18 用户确认内网两批尚未开始，要求直接排除且保留数据，不增加页面说明或排除状态。
  本次验证名单排除 `RPAADM_002192 / siem / T_GBD_zeus_data`，批量与单条入口一致；
  第一批不学习、第二批不验证这7条邮件，原始语料及其他告警的分批归属保持不变。
  这仅落实验证范围，邮件专项处置兼容仍保持下述 Deferred 状态。
