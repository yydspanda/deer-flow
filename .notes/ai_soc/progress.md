...
- **Restart follow-up:** 第二批新增“重新配置并全部重跑”；现有轮次JSON标记旧队列退役，事务校验全批活动、Host保存设置与稳定token，成功/失败/排队全部新建任务，旧结果和第一批/Memory保留。队列、详情和筛选按最新任务显示，readiness覆盖与分页一致，固定历史审计继续显式读取。17项列表/批次最终回归、62项旧记录/经验兼容、78项前端单测、2项Host/LAN浏览器、完整前端check及53项交付清单/手册检查通过；新增27项后端及既有38项快捷/执行/仓储回归通过，最终HTTP与历史耗时专项复验通过；包含跨标签去重、旧队列拒绝恢复、主机时钟回拨、旧JSON默认字段与旧耗时保留。实现验证未修改内网或调用真实模型；用户随后授权先提交推送再打包，沿用保留数据升级并重跑绑定提交的兼容门禁。
- **Delivery boundary:** 原清理方案已于09-22被用户取消，内网仅预检拒绝、没有执行删除。后续改为网页全量重跑并保留旧结果；本条记录仅保留此前已交付维护工具的历史事实。第一批和审核经验不重建。
