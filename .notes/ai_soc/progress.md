...
- **Task:** `PI-03E`
- **Status:** `Done`
- **Outcome:** Host页面恢复各批服务器保存设置，未提交草稿按批隔离；批次阻塞只取当前queued/active，历史终态不阻挡恢复。新增DEV停服维护工具，默认只读预览；显式apply先复制并校验完整SQLite家族，再事务清理固定实验第二批全部尝试和结果。保护第一批与审核Memory；共享scalar/JSON来源、缺失配置、活动任务或自定义触发器均拒绝清理。新第二批经原有API提交第一批完整设置与新审核经验快照，不重写旧冻结任务。
- **Verification:** 后端批次41项、清理边界24项、前端组件19项及完整前端check通过；维护包装/Host/交付清单144项通过，含真实SQLite清理后第一批逐行不变、备份恢复及文档命令完整5项设置/重复提交回归。审核发现的JSON转义、结果ID反向引用及缺失配置漏洞均经TDD修复。交付清单与独立可复制操作文档同步；ae4246bb交付前62项兼容门禁通过。后续内网46apply/1off历史触发预览拒绝，补显式基准选择及外置工具路径边界：清理31项、维护25项回归通过；旧第一批历史完整保留，未修改应用Runtime。
