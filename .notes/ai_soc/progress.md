...
### 2026-09-22 — Restore saved batch settings and prepare validation-only recovery
- **Task:** `PI-03E`
- **Status:** `Done`
- **Outcome:** Host页面恢复各批服务器保存设置，未提交草稿按批隔离；批次阻塞只取当前queued/active，历史终态不阻挡恢复。新增DEV停服维护工具，默认只读预览；显式apply先复制并校验完整SQLite家族，再事务清理固定实验第二批全部尝试和结果。保护第一批与审核Memory；共享scalar/JSON来源、缺失配置、活动任务或自定义触发器均拒绝清理；唯一显式例外为归一化维护项的可空代表Run链接：保留整项，核对身份后仅清空scalar/JSON根字段，其他引用仍受保护，更新和删除共用事务及回滚。新第二批经原有API提交第一批完整设置与新审核经验快照，不重写旧冻结任务。
- **Verification:** 后端批次41项、清理边界24项、前端组件19项及完整前端check通过；维护包装/Host/交付清单144项通过，含真实SQLite清理后第一批逐行不变、备份恢复及文档命令完整5项设置/重复提交回归。审核发现的JSON转义、结果ID反向引用及缺失配置漏洞均经TDD修复。交付清单与独立可复制操作文档同步；ae4246bb交付前62项兼容门禁通过。后续内网46apply/1off历史触发预览拒绝，补显式基准选择及外置工具路径边界：清理31项、维护25项回归通过；旧第一批历史完整保留，未修改应用Runtime。针对实际NMI阻断新增完整行保留、备份还原、失败回滚、身份和嵌套引用/触发器保护回归；最终执行数量及绑定提交的兼容结果记录在独立维护包MANIFEST。操作说明改为单篇1～5步，不再引用旧教程。
- **Delivery boundary:** 用户明确授权丢弃第二批旧结果；当前未修改内网数据库、未运行真实模型。普通升级保留数据，实际清理及12284条第二批重跑仍需内网执行；第一批和审核经验不重建。
...
- **Task:** `PI-03E`
- **Status:** `Done`
- **Outcome:** 用户确认内网已成功暂停后，按要求在运行设置中开放部署本机保存 1～8 并发；局域网同事只读并沿用设置。两批、手动任务与后台草稿共享容量，调低排空已开始任务后补位，调高领取原队列；小型配置文件与 SOC DB 同目录原子保存，不写业务库、不改轮次行为配置或 Memory，重启及保留数据升级可恢复。页面说明降低并发时暂时出现的 8/2，不把已有任务中断或重复执行。
- **SQLite compatibility:** 内网 Python SQLite 3.45.3 的 DELETE 模式继续保留；只有含官方 WAL-reset 修复的 SQLite 才启用 WAL，旧版遇到已有 WAL 明确要求先升级，不在线转模式。30秒等待和仅结果持久化的有界重试保持，不重调模型。
- **Verification:** 受影响批次/工作台/权限回归136项（含新配置单测34项与8→2→8集成8项）、旧入口/草稿边界3项、SQLite连接与争用36项、旧记录/经验兼容62项、Host启动与数据保留77项、交付清单及Runbook53项全部通过。DELETE模式8任务/页面读写/暂停场景另连续5次通过；前端63项、ESLint/TypeScript、真实Chromium交互通过。Ruff格式/静态检查与进度治理通过。
- **Boundary:** 以上为隔离数据和模拟模型证据，不代表内网长期容量验收；实现验收时未打包、提交或部署，未操作业务数据库；用户随后授权先提交推送，再重建保留数据交付，最终回执另存于交付目录。SQLite仍可能等待外部长事务，不承诺任意争用均不会超时。
### 2026-09-20 — Preserve reviewed experience matching and filter oversized new learning features
- **Task:** `PI-03E`
- **Status:** `Done`
- **Outcome:** 按用户确认撤回 `bb03200b` 中长实体摘要化，保留旧查询及对象关联匹配语义；进一步改为仅在新学习构造中过滤超过 512 字符的实体特征，其余合法特征继续走原有稳定锚点、质量和审核门禁，新条件目录排除超长值并拒绝选入。不新增摘要、截断值或替代特征，不过滤第二批查询输入，也不删除已审核条件。成功研判、人工纠正与审计、主模型输入上限和真实进度修复保留；已有 Run、Observation、候选和审核记录不改写。
- **Verification:** 243 项后端聚焦及回归通过，覆盖自动积累、人工提升/纠正/备注、审核目录与草稿、SQLite 审核和后续匹配、批次执行及历史重放；Ruff 格式与静态检查通过。对照内网基线 `2b4d3e43`，已有特征提取函数及查询、对象关联、匹配和精确复用实现保持一致。只读加载 `2506195 / RUN-E803E6349C7D`，在隔离内存库过滤 1 个 1565 字符实体，保留 8 个实体并正常积累 1 个样本；原 Run 与匹配查询未变，未绕过候选门槛。无真实模型调用或实际数据库写入；未重建或部署内网包。
...
本轮补充：两批页面用语改为“验证经验复用／其他告警测试”，恢复新轮次四个开关并保存实际选择；旧轮次配置不变。10项组件与5项浏览器回归通过，前端已恢复演示模式的预构建服务。按用户最新要求暂停内网打包，后续代码、私有配置、数据仍分开。见月度 `EXP-20260918-corpus-batch-controls`。
缺表收尾：批次接口新增只读结构检查，返回中文升级提示而非500，配置与开关继续可见；11项API/仓储及5项迁移/Host启动测试通过。外网业务库未迁移或清空，仍等待用户决定；本机接口验证503提示与200配置正常。见月度 `EXP-20260918-corpus-schema-readiness`。
报告收尾：CLI/报告与网页名称统一，离线对照拒绝跨批次或未知批次；报告/CLI/临时库完整闭环22项通过。打包继续暂停，本机升级等待确认，真实运行仍留内网。见月度 `EXP-20260918-corpus-report-alignment`。
