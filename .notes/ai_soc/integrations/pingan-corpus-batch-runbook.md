...
按本次交付主 Runbook 完成备份、安装、语料落位、依赖安装、SOC DEV 重置、启动和模型 Smoke。
用户已明确要求本次从零验证；若已按主 Runbook 重置成功，直接进入本手册第2节，不重复重置、安装或启动。
...
普通升级必须保留库；本次用户明确选择重置 SOC DEV。账号库、STG库、源语料和密钥不属于实验清理范围。

### 1.1 仅本次从零开始：重置 SOC DEV 数据

已在本次交付主 Runbook 中完成重置的部署跳过本节，避免清空刚产生的新结果。
本节仅供仍持有旧 SOC DEV 数据、尚未执行该重置的部署使用。
