...
| Real `asset.locate` | **In progress / internal evidence pending** | `D12-B` | 自包含 ZEUS signer + Agent Platform HTTP client、ZEUS PRD/STG 与 YHSYS PRD private-profile preparer、固定 legacy operator、Apple Silicon Host DEV/STG、preflight/direct seven-case runner 与 MCP/Dispatcher/evidence/shared-context acceptance runner 已实现；受治理映射为项目 DEV -> ZEUS PRD、项目 STG -> ZEUS STG |
