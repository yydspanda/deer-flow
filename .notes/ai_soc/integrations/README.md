...
| Real `asset.locate` | **In progress / ZEUS PRD evidence pending** | `D12-B` | 自包含 ZEUS signer + Agent Platform HTTP client、共享 ZEUS/YHSYS PRD private-profile preparer、固定 legacy operator、Apple Silicon Host DEV/STG、preflight/direct seven-case runner 与 MCP/Dispatcher/evidence/shared-context acceptance runner 已实现；当前从 DEV Host 执行 PRD lifecycle/provider 验收 |
