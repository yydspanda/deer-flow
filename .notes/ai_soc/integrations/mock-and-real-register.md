...
| Automatic read-only investigation orchestration | **D1-D3 deterministic production code; D4 current** | `backend/soc_agent/contracts/enrichment.py`、`backend/soc_agent/core/enrichment.py`、`backend/soc_agent/core/investigation_workflow.py`、`backend/soc_agent/application/enrichment.py`、migration `0019_enrichment_executions` | 默认关闭；typed plan 经同一 Dispatcher/Registry；execution/attempt/evidence 可持久化；实际 `mocked` 在 evidence 前校验；Kafka/internal batch 仅显式 opt-in；幂等、failure/not-found、bounded retry、stale recovery 和 linked replay 已实现 | D4 补 shadow report/telemetry/addendum。真实 Provider 仍须各自 `mocked=false` 验收；orchestration 完成不等于 PingAn asset/TI/tag 已真实接通，且任何阶段都不改基础 verdict/close/memory/automation |
