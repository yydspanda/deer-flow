...
| `security_tag.lookup` | in-memory 标签/授权/白名单 mock adapter | `backend/soc_agent/actions/adapters.py` | 验证授权扫描、演练、维护窗口、白名单等标签 evidence 形态 | `PI-01B1` 替换为真实安全标签 provider；标签查询本身不等于权威授权事实已同步 |
