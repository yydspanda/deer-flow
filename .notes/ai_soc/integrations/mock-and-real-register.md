...
| PI-01E paired shadow evaluator | **Deterministic validation code; not a Provider or mock** | `validation/compact_zeus/internal_batch/evaluate_pingan_shadow.py` | 无外部 IO 地核对 paired batch/cohort/config、real/mock、pre-LLM compatibility、P95/review/schema/measurement gaps 与零越权副作用 | 仍需内网真实 5/50/all 产物；技术 pass 不证明模型准确、不自动扩容、不关闭 D12-B/PI-01A/B1 gate |
