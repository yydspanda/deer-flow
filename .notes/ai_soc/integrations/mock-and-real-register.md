...
| `D12-B` PingAn internal validation | **Code/config/offline toolchain/acceptance runners prepared; real evidence pending** | `backend/samples/pingan_dev/`、`backend/scripts/soc_pingan_dev_preflight.py`、`backend/scripts/soc_pingan_asset_direct_smoke.py`、`backend/scripts/soc_pingan_d12b_matrix.py`、`backend/scripts/soc_pingan_d12b_evidence.py` | Host Runtime 与远端目标在底层分离、在部署 profile 中受治理组合：项目 DEV -> ZEUS PRD + Agent Platform PRD，项目 STG -> ZEUS STG + Agent Platform STG；两者分别使用独立 SQLite/Memory/Policy/Automation scope，只有 DEV 启用 Workbench。旧源码只提供 STG origin，缺少 STG `YHSYS` identity/IDs 时切换 fail closed。真实值保存在 Git-ignored mode-`0600` private profile/case file；plan/fake 不发请求，live 必须显式确认 | 从匹配环境完成 direct + MCP + persisted evidence/readback；至少一项 `mocked=false` 且 deployed Web/TUI smoke 通过后才改变 real-provider 状态 |
...
| SQL/in-memory repositories in tests | in-memory repository 或 SQLite 单元测试 | `backend/soc_agent/*/repository.py`、`backend/tests/` | 单元测试和无 DB 局部 wiring；PingAn 内网 `dev|stg` 演练分别使用 `{database.sqlite_dir}/soc_agent_dev.db` / `soc_agent_stg.db` | 最终生产/准生产验收必须走 PostgreSQL migration + SQLAlchemy repository；当前内网联调不收集 PostgreSQL 参数 |
...
|---|---|---|---|---|
| `RID-01 D12-B asset.locate` | 同一 Provider/MCP/action 代码 + fake transport，`mocked=true`；自包含 workflow HTTP client、ZEUS/Agent Platform 受治理 target profile 与离线 Mac toolchain 已准备；`message.by` 固定为旧值 | 匹配部署环境的两类 endpoint/allowlist/credential/workflow IDs（项目 DEV -> 两者 PRD、项目 STG -> 两者 STG）、批准的 hit/not-found/UM/ambiguous cases；缺少 STG `YHSYS` profile 时 fail closed | preflight、direct matrix、MCP smoke、Dispatcher persistence/readback、Web/TUI readback | 至少一个真实 hit，完整失败矩阵，所有成功 evidence `mocked=false`，基础 verdict/Review 不变 |
