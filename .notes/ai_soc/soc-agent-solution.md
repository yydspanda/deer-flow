...
真实 Kafka consumer adapter 规划见 `.notes/ai_soc/kafka-consumer-adapter-plan.md`，生产运行约定见 `.notes/ai_soc/soc-daemon-production-runbook.md`，worker pool / concurrency 规划见 `.notes/ai_soc/kafka-worker-pool-concurrency-plan.md`。Prometheus `/metrics` exporter 暂缓，先依赖 JSONL 日志采集。下一步若继续 Kafka daemon 可靠性，应做 WorkerPoolResult contract，而不是直接开线程。
