...
真实 Kafka consumer adapter 规划见 `.notes/ai_soc/kafka-consumer-adapter-plan.md`，生产运行约定见 `.notes/ai_soc/soc-daemon-production-runbook.md`。下一刀进入 production overlay planning；Prometheus `/metrics` exporter 暂缓，先依赖 JSONL 日志采集。
