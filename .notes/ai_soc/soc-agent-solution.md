...
真实 Kafka consumer adapter 规划见 `.notes/ai_soc/kafka-consumer-adapter-plan.md`，生产运行约定见 `.notes/ai_soc/soc-daemon-production-runbook.md`。Prometheus `/metrics` exporter 暂缓，先依赖 JSONL 日志采集。下一步应在真实环境参数验证和 worker pool/concurrency planning 中二选一。
