...
- Chat entry：
  ...
  - 当前已新增 `backend/soc_agent/actions/mcp.py` skeleton 和 `DeerFlowCachedMcpToolProvider`：固定 `SocMcpToolProviderPort`、`SocMcpToolActionAdapter`、read-only MCP descriptor helper、explicit config builder、DeerFlow cached MCP tool provider 和 fake-provider tests；尚未做 dev/staging read-only live smoke。
