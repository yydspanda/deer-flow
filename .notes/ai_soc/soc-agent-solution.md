...
#### Unified Learning Entry / 统一经验入口

人工提前提炼与自动达标提炼，共用 `SocMemoryService.propose_candidate(coordinate_learning=True)`。
自动门槛仍需同窗口 5 条独立有效样本、结论一致性和稳定锚点，不是看到计数 5 就直接生成 Memory。
入口协调只决定“去哪一份经验继续处理”，不替代审核，不修改本次告警判断。

| 当前情况 | 页面主操作 | 后续样本 / 再次点击 |
|---|---|---|
| 未有适用候选，未达自动质量门 | 提炼经验 | 可人工提前发起；自动继续积累 |
| 已有人工或自动待审候选 | 审核人工提炼经验 / 审核同类经验 | 返回同一候选，不再制造另一份审核任务 |
| 已审核并开放参考 / 精确复用 | 查看 / 修订经验 | 追加观察，不重写原业务结论或重新审核 |
| 已沉淀但暂停、到期 | 查看 / 修订经验 | 明示暂停或需复查，不自动恢复使用 |
| 已有待审修订 | 继续审核修订 | 优先进入修订，不跳回旧候选或重复新建 |
| 本窗口已放弃 / 过期 / 废止候选 | 查看记录 | 不反复自动打扰；可从记录显式治理 |
| 后续独立窗口再次满足条件 | 按当前范围重新判断 | 旧终态不永久阻挡；保留原历史 |
| 已有经验的 IP 等限制未覆盖新样本 | 提炼新范围经验 | 相似召回不等于适用，不拦住新场景 |

范围身份包含租户、数据类别、Profile 与类型化适用条件；不按告警标题或人工/自动来源分两套。
已审核范围以 Memory Record 为准，不能用原 Candidate 的宽范围覆盖人工收窄结果。
自动与人工写入在同一治理事务锁内重新判定；旧字段布局只读兼容，无需清库或 Profile 升级。
服务端 `SocMemoryLearningView` 输出状态、说明、主操作和目标编号，告警演练、告警详情与经验中心
使用该投影。旧历史重复项不会自动合并、确认或删除；原有范围冲突审核仍有效。
完成通知只报告执行结果，经验状态栏保留唯一审核入口；同一候选出现在多条告警下时只需审核一次。
人工提炼标明来源告警，自动提炼标明同类样本来源。两者审核并开放使用后均按适用条件服务新告警，
不因提炼来源推断适用范围宽窄，也不把同一候选的两个页面入口解释为两个审核任务。
...
经验详情和修订页面也可直接“废止这条经验”，无需绕回确认记录或先创建修订候选。两个入口复用
原有 source Candidate 的 `review(decision=deprecate)` 权限、事务及审计，需填写原因并确认；
取消不写入，失败保留原因，成功刷新为“已废止”并保留历史。已有待审修订时先处理该修订，
不由新入口静默取消；已废止记录不再展示修订、开放使用或未来复用能力操作。
...
两批持久轮次、统一服务端调度、CLI、实验聚合与固定结果导出已接入；隔离完整闭环、浏览器和合成容量验收通过。
第一批不使用已有 Memory，第二批冻结第一批审核经验且不自动学习；审计与结果按轮次 Run 固定。
最终独立代码/数据交付及真实 Mac 性能仍待验收，未清库、未关闭真实内网门禁。
只读分批准备已实现：CLI 核验源 PKL/索引/载荷库，按静态组内事件时间生成受保护草案。
已确认第一批每组最多10条：15,288条分为第一批3,002、第二批主要8,522、补充3,764。
第一批已按本实验及冻结配置的同一实际模式统一积累，保留真实时间和普通30天窗口；
页面与后台共享固定轮次。批量AI草稿、速率/预计时长分项和最终内网分包仍待收尾，规则确认不等于授权真实运行。
外网仅开发与模拟，真实批跑、经验审核及效果对比全部留在内网 Mac DEV；静态组不冒充语义核对后的模式。
