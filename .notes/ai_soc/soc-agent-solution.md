...
真实 Kafka consumer adapter 规划见 `.notes/ai_soc/kafka-consumer-adapter-plan.md`，生产运行约定见 `.notes/ai_soc/soc-daemon-production-runbook.md`。下一刀优先补 JSONL metric sink 或 isolated run-mode smoke，避免默认 topic 历史消息干扰验收。
