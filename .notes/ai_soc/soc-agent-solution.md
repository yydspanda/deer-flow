...
截至 2026-07-08，Phase 1 的 Runtime、ReviewQueue、Lead Agent entry、approval boundary、read-only action evidence、Kafka daemon 基线和本地 MCP mock/smoke 已基本收口；`SocCorrelationService`、PingAn PA-01..PA-11、APT/EDR/HIDS domain triage、只读 Main Orchestrator demo、外部反馈可见性、候选记忆 DB/API/ReviewQueue 可见性、候选记忆评审状态机、confirmed-memory retrieval policy MVP、Web/TUI visible investigation MVP 和 persistent demo script MVP 已完成。`soc demo run [all|apt|edr|hids]` 现在能用一条可重复命令生成可打开的 ReviewQueue investigation chain。`PA-12` 是真实 PingAn dev/staging MCP/API replacement，受 endpoint/凭证门控，不作为当前 Alpha 主线阻塞项。当前主线转向 memory candidate 来源闭环和通用安全场景识别；F5/WAF 只是可选 source/adapter 示例，不是固定下一阶段目标。Memory candidate 来源闭环已完成第一步：`SocMemoryCandidateSourceBridge` 统一承接 correction 和 domain finding -> pending candidate，后续继续接 Kafka/Lead Agent proposal/review note 等来源；分析师对 domain/scenario finding 的反馈也能进入 pending candidate，但仍不能直接 confirmed。Generic scenario recognition 已完成 deterministic MVP：`SocDomainFinding` 现在带 `scenario_key`、`evidence_profile`、`current_conclusion` 和 `human_checklist`，并按 Evidence Fusion First 使用 raw/canonical alert、历史相似预警、外部反馈、confirmed memory 和可用工具证据；工具证据缺失不会阻断当前结论。若厂商/上游提供了场景提示但内部场景库未命中，系统会输出 `vendor.unmapped` 候选 finding，保留上游提示、证据缺口和人工核查清单，但不把它当作最终 operational verdict 或 confirmed memory。Scenario taxonomy 已暴露稳定 version/keys/snapshot，PingAn domain eval 和 vendor-neutral `soc eval scenarios` 均可输出 covered/missing/unmapped 计数；`soc eval scenarios --baseline-json` 可生成 replay diff 报告。WebShell 规则已移除过宽 `jsp/php/upload` 单词匹配，降低误报。
...
[Done] Phase 1 reliable runtime / review / approval / Kafka baseline
  ...
  -> [Done] APT/EDR/HIDS source handlers done; generic scenario recognition deterministic MVP + vendor.unmapped fallback
  -> [Done] Web/TUI visible investigation MVP
  -> [Done] Demo / Eval Script MVP
  -> [Partial] Memory candidate source integration: correction + domain finding + analyst feedback bridge done; Kafka/Lead Agent/review note pending
  -> [Partial] Generic security scenario recognition deterministic MVP: evidence fusion + current conclusion + vendor.unmapped fallback + vendor-neutral taxonomy eval/replay diff baseline; bounded LLM recognizer/custom taxonomy pending
```
...
- Chat entry：
  ...
  - Web/TUI approval inbox 已展示 proposal 来源、payload 和 context refs，审批人不需要只看 action 名称。
  - 当前已落地 `SocActionAdapterRegistry` contract：真实 EDR/F5/SOAR/MCP 动作必须通过 `SocAgentActionAdapterDescriptor` 注册 `route/action`、side-effect、必需 payload/context 字段和 dry-run/execute 能力；未注册动作不能执行。
  - 当前已把 `SocAgentApprovalService.dry_run_approved_action()` 接入可选 action adapter registry：有 registry 时，dry-run 会在 approval token 校验后继续校验 adapter allowlist、proposal payload 和 context refs；无 registry 时仍保持原 token-only dry-run。
  - 当前已把 `SocAgentApprovalService.execute_approved_action()` 接入可选 action adapter execute preflight：有 registry 时，execute 会在消费 token 前校验 adapter 存在、支持 execute、payload/context refs 满足要求；当前仍不调用真实 adapter.execute。
  - 当前已新增第一个具体 read-only adapter：`asset.lookup` / `InMemoryAssetLookupActionAdapter`，用于本地/测试验证资产查询 adapter contract；它是只读 in-memory/static inventory，不是生产资产系统接入。
  ...
  - 当前已把 SOC Lead Agent read-only proposal bridge 接入 action proposal boundary：Lead Agent 输出 `asset.lookup` / `asset.locate` proposal 时，只有注入 read-only router/dispatcher/registry 后才会产生 `soc.route_decision`、`soc.permission_decision`、`soc.action_result`；高风险 proposal 仍进入 approval inbox。
  ...
  - 当前已新增 `backend/soc_agent/actions/mcp.py` skeleton 和 `DeerFlowCachedMcpToolProvider`：固定 `SocMcpToolProviderPort`、`SocMcpToolActionAdapter`、read-only MCP descriptor helper、explicit config builder、DeerFlow cached MCP tool provider、显式 JSON/YAML config loader、`SocMcpActionSmokeReport`、`SocMcpToolInventoryReport`、`soc mcp smoke/tools`、fake-provider tests 和本地真实 stdio MCP smoke fixture；`mcp.server` 已作为执行路由绑定传入 provider / one-shot smoke，避免依赖工具名前缀猜 server；本地 mock 已提供 `asset.locate -> soc_dev_asset_locate`，用于模拟 Zeus/CMDB/asset_to_bu 远程归属定位；真实 CMDB/EDR dev/staging MCP 参数待替换接入。
  - 当前已新增 `soc-asset-extraction` skill：资产提取、角色标注和 `asset.locate` proposal 由 skill 约束；skill 不执行远程调用，也不宣称归属结果，归属/BU 定位必须回到 MCP-backed action adapter。
  - 当前已新增 read-only action evidence bridge：`asset.lookup` / `asset.locate` 等只读 adapter 成功结果可记录为 `InvestigationEvidence`，并进入 ReviewQueue context、SOC Lead Agent bounded artifact、TUI/Web 只读展示；它只是调查证据，不自动改 verdict，不升级为 confirmed memory，也不能触发处置。
  - 当前已把 `InvestigationEvidence` 落到 PostgreSQL SOC business store：`soc_investigation_evidence` 表、SQLAlchemy repository、Gateway ReviewQueue context、CLI/TUI/Lead Agent read-only dispatcher 使用同一 repository，支持 Web/TUI/daemon/Lead Agent 跨进程复用只读查询结果。
  - 当前已新增 `endpoint.process_tree.lookup` in-memory/mock read-only adapter，用于在没有真实 EDR MCP 时验证 endpoint process-tree 调查、action evidence 写入和 Lead Agent proposal bridge；真实 EDR MCP 可用前不阻塞主线。
  - SOC Lead Agent bounded context 已明确要求先复用已有 `action_evidence`，避免重复提出同类 read-only lookup。
  - `soc chat tui --lead-agent --mcp-action-config PATH` 可显式注入 MCP-backed read-only action registry；默认不自动加载本地 MCP adapter config。
  - SOC Lead Agent profile 不配置自定义 `mcp` 字段。DeerFlow custom-agent profile 只负责 `skills/tool_groups/SOUL.md` 等 profile 语义；MCP server 连接由 DeerFlow `extensions_config.json` / `mcp_config.json` 管理，SOC action 到 MCP tool 的业务绑定由 action adapter allowlist 管理。
...
7. **SOC Agent chat stream contract**
   ...
   - 当前已落地真实 Kafka broker consumer 基础链路：`KafkaRecord -> SocDaemonMessage` mapper、`SocKafkaConsumerRunner`、`KafkaConsumerSettings`、`ConfluentKafkaConsumerPort`、local Redpanda smoke、`soc daemon status` readiness 和 `soc daemon run` 长驻 loop shell。`soc daemon run` 已具备 metrics 与 adapter/runtime error backoff，并支持 `--metric-jsonl stdout|stderr` 运行中事件输出。生产 entrypoint / healthcheck 已固定在 `backend/scripts/soc_daemon_entrypoint.sh` 和 `backend/scripts/soc_daemon_healthcheck.sh`。`docker/docker-compose.soc-daemon.yaml` 提供显式 opt-in compose overlay，默认 build extra 为 `postgres,kafka`；`backend/Dockerfile` 支持 multi-extra build arg。`docker/k8s/soc-daemon.yaml` 提供显式 opt-in K8s deployment contract 示例，ConfigMap/Secret/probes/resources/logging 标签已固定。`soc_kafka_smoke.py --mode run` 已覆盖生产入口的真实 broker smoke。`KafkaWorkerResult` / `SocKafkaWorker` 已固定未来 worker pool 的返回结构，但当前不启动并发。Kafka adapter 仍只做 ingestion，不拥有分析逻辑。
...
| 来源 | 生成对象 | 默认状态 | 说明 |
|---|---|---|---|
| `soc correct` / ReviewQueue correction | correction fact / lesson candidate | `pending_review` | 已通过 `SocMemoryCandidateSourceBridge` 接入；分析师明确确认时可信度较高，但仍要带 evidence |
...
| DomainTriageResult | topic/scenario candidate | `pending_review` | 已有 bridge/factory，可由显式 service/entry 调用；反弹 shell、webshell、横向移动、命令执行、恶意外联等场景 finding 稳定后继续扩展；APT/EDR/HIDS/F5 只是输入来源或 handler 起点 |
...
6. Done：Confirmed memory retrieval policy MVP 已完成，新增 `SocMemoryQuery`、`SocMemoryRetrievalResult`、match reason、score、token budget、retrieval-enabled gate、CLI/API search 和 `InvestigationContext.relevant_memories`；仍不做 prompt 注入。
