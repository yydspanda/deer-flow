...
| `CONS-03` | Data-gated live feed | Product/lifecycle diagrams show Zeus/ITSM/SOAR -> adapter -> `SocExternalDispositionService` | Canonical Gateway ingress, mapper contract, SQL repository, correction/exact-used-Memory-feedback/outcome bridges and tests exist; a real Zeus/ITSM/SOAR authenticated feed remains unavailable | `core/external_disposition.py`, `routers/soc_external_disposition.py`, `AC-08/AC-09`. Current product boundary is implemented canonical ingress; real feed ownership remains data-gated |
...
| `CONS-15` | Resolved by `BG-P1-03` | Engineering contract says LLM request metadata must be recorded before the non-rollbackable provider call, then final decision is written | Persisted analysis now commits `AnalysisRequestJournal` on the exact pre-provider Runtime hook; timeout/final bundle finalize it, while process loss or bundle rollback leaves a discoverable running row recoverable as interrupted + linked replay | `AnalysisRequestJournal`; `DeterministicAnalysisRuntime.analyze_journaled()`; `SocAnalysisService.recover()`; process-loss/timeout/rollback tests |
