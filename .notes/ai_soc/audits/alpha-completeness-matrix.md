...
| `AC-07` | External disposition canonical service and SQL persistence | Complete | - | Mapping, idempotency, target resolution, correction/exact-used-Memory-feedback/outcome bridge and durable table exist | Maintain |
...
| `AC-40` | Correction and review-note Memory admission | Complete | - | Both pass `MemoryAdmissionService`; ordinary notes/corrections remain `observed_only` unless explicit human promotion, substantive reason and reusable facet all pass. The separately authenticated run-to-candidate command uses the explicit action plus exact run/alert lineage instead of mandatory prose | Maintain |
| `AC-41` | Domain/Lead Agent/Kafka lesson capture workflow | Complete (scoped) | - | Lead Agent requires explicit analyst acceptance; Kafka/batch writes observations and only quality-gated cohorts create one candidate; domain bridge remains explicit/service-only | Maintain; do not add noisy automatic domain capture |
