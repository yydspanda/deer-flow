...
2026-09-11 同步上游 `452d09b9`，保留 SOC 研判、经验审核和免登录 DEV 演示入口。
上游依赖与数据库迁移同步更新；本地备份和兼容验证见 [9 月归档](../archive/ai_soc/progress/2026-09.md)。

运营可见的研判摘要、企业策略理由、处理建议、核查项和经验正文统一要求简体中文。
IP、路径、规则码、HTTP/SQL 等技术名称及原始审计 JSON 保持原样。历史模型原文不回填翻译，
重跑才会按新 Prompt 生成；固定阶段说明可直接中文展示，不改变历史结论。见
[`从研判到处置`](governance/decision-to-policy-flow.md)。
...
经验审核中，选择最终判断即可请求 AI 起草；业务事实可选，只补充已核实的业务情况。
生成后各项内容直接可编辑，也可预览；确认时提交的是修改后的内容。新候选和修订候选使用同一流程。
重新生成前会确认是否覆盖草稿，生成失败保留已有修改；适用条件仍在匹配范围区域调整，不靠改文字改变匹配。

经验详情和候选审核以可读的 **适用告警** 展示检测规则、产品和核心行为，Hash 留在展开的技术详情。
后端核对指纹组成后才展开；缺少明细的旧记录不猜测。页面分为 **必需匹配条件** 和 **可选匹配条件**，
只去掉已被必需条件覆盖的重复值，不隐藏同组剩余条件。审核时可逐值增加限制；同组多选接受任意一项，
不同组需同时满足。已确认经验只读，未增加的条件仍可查看。历史记录、指纹及使用权限不变。
与唯一必需检测规则同源的 `rule:<Hash>` 不再作为独立可选条件；旧记录只读去重，原始索引和技术详情保留。
...
| 审阅页面提速与两批经验验证 | [`architecture/corpus-memory-batch-validation-design.md`](architecture/corpus-memory-batch-validation-design.md) | 页面/持久批跑/实验积累及冻结验证已接入，隔离闭环与容量通过；真实内网及最终交付待完成，未清库 |
...
│   └── corpus-memory-batch-validation-design.md # 性能/分批/持久批跑已接入；真实验收仅内网Mac DEV
├── reporting/
...
└── governance/
    ...
    ├── effectiveness-and-rule-optimization.md # 最终真值、Rule Code/同类行为/Memory 效能和算力闭环
...
| `governance/effectiveness-and-rule-optimization.md` | 最终真值、准确率/漏报/自动化、Rule Code→同类行为→Memory 效果与算力优化的统一口径 | 埋点、指标公式、规则建议阈值、最终状态来源或快速路径边界变化时更新 |
