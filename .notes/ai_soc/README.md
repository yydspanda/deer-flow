...
| `kafka-worker-pool-concurrency-plan.md` | Kafka daemon worker pool / 并发处理规划，固定 offset、幂等、backpressure 边界 |
...
后续设计只维护 `soc-agent-solution.md`，开发进度只维护 `progress.md`。独立会议结论或专题决策可以新增单页说明，但必须在本 README 建入口，并在需要进入开发路线时同步回 `soc-agent-solution.md`。
