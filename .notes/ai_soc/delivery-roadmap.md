...
|---|---|---|---|---|
| 1 | `D12-B` | Real asset provider / 真实资产定位 | 仅在 PingAn integration 内接 ZEUS + Agent Platform workflow/UM 降级链；受治理映射固定为项目 `DEV -> 两者 PRD`、项目 `STG -> 两者 STG`，缺失 STG `YHSYS` 私有 profile 时 fail closed；generic 层仍只认识 `asset.locate`；现有 acceptance runner 复用 Dispatcher/Review Context，不新增 Runtime 节点 | direct + MCP success/not-found/auth/timeout/ambiguous；至少一项 `mocked=false`；持久化证据可从共享 Review/Lead Agent context 回读且基础 Run/Review 不变；deployed Web/TUI smoke 单独通过 |
...
| 12 | `PI-01H` | Legacy ZEUS compatibility execution plane / 旧 ZEUS 兼容执行面 | **External implementation done / Internal gate open**: 保留 `/workflow/task`、`/task/task_status`、`getAlertBrief` 与 callback；durable Job、lease Worker、统一 Runtime、legacy projection、Outbox/attempt audit、项目模型网关、Host sidecar、fake E2E、只读 lifecycle/signature smoke 和脱敏 live acceptance runner 已实现 | 外网幂等、租约恢复、签名 wire-body 一致性、回调隔离和容量门回归通过；真实 DEV 必须先证明 `code=200/status=1`，再由 local runner 验证组件组合，并由 ZEUS 上游真实发起、旧页面回读和负载证据最终关闭门禁 |

Current status / 当前状态：`D12-B` 为 `Active / internal evidence pending`；其代码、私有 case matrix 和验收门槛全部保留。`PI-01A` 已完成 production-shaped PingAn Provider、stdio MCP、generic action config、显式 fake smoke，以及 hit/not-found/partial/error/freshness/trim/lineage/persistence 回归；真实内网 `mocked=false` smoke 与实际响应字段复核仍是退出门槛，因此不能标记 Done。`PI-01B1` 已完成 `/public/searchTagContent` Provider/MCP/action 的外网可验证部分，保留 active/expired/inactive/conflicted/unknown/out-of-scope/unusable/not-found 语义；真实 DEV 响应、对象类型、无过期时间语义与持久化 `mocked=false` 证据仍待内网验收。
...
| `PI-03E` | Semantic normalization and adaptation quality / 语义核对与适配质量 | **In progress 2026-09-16**: [常规模型核对与按需质量检查](architecture/normalization-assistance-design.md)及 [Memory 范围整改](architecture/memory-reuse-scope-remediation.md)；新 apply Run 使用 Profile 9 / features v7，稳定检测投影、完整覆盖判断、细分审核及范围生命周期已通过本机结构回归，旧 Profile 7/8 冻结。真实抽取/匹配质量与用户完整闭环仍待验收 | 启用后每条有效输入常规核对，不以代码异常门控。新增未覆盖行为进入参考研判；更具体的已审核范围优先。同类候选实体按需分页，不自动迁移/审核 Memory。质量按需检查；仍禁止 Runtime 自修改和自动发布映射 |
