...
| `PI-03E` | Adaptive parser governance / 自适应解析治理 | **Design proposed 2026-09-14 / Not implemented**: [覆盖检查与有界事实补充](architecture/normalization-assistance-design.md)；复用归一化运维，后续按日志类型验证并固化规则 | 拟将原“禁止单告警 LLM 解析”收缩为允许受控、可追溯的当前事实抽取；线上未启用。仍禁止 Runtime 自修改、自动发布映射；规则发布继续要求 cohort、双跑、审核、灰度与回滚 |
