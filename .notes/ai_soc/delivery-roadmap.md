...
| `BG` Close Blocking Gaps | **Current / BG-P1-04 In Progress** | 只修审计确认的代码可控 P0/P1 阻塞项 | Alpha 端到端验收通过 |
...
| `BG-02` | Close P1 + E2E acceptance / 修复 P1 与端到端验收 | **In progress**: `BG-P1-01..03` 已完成，当前执行 `BG-P1-04`，之后执行 `BG-P1-05` | 结果可重复；失败语义、offset、review、audit 和 memory boundary 符合契约 |
...
In Progress:  BG-P1-04 - Governed memory activation (AC-39)
Next:         Add role/reason/audit/version-controlled retrieval enable/disable across service, CLI, API and Web
Blocked by:   None for BG-P1-04; automatic lesson capture and production memory quality remain deferred/data-gated
```
