...
| `PI-03E` | Semantic normalization and adaptation quality / 语义核对与适配质量 | **In progress 2026-09-14**: [常规模型核对与按需质量检查](architecture/normalization-assistance-design.md)；v2 类型化对象/检测/补充事实、Run/journal/恢复、canonical 消费及 PingAn opt-in Profile 8 / v6 已接入并离线验证，默认 off；真实抽取/匹配质量与检查入口未完成验收 | 启用后每条有效输入常规核对，不以代码异常门控。质量记录自动保存、人工按需检查；不要求逐条修 Adapter。仍禁止 Runtime 自修改、自动发布映射；未来规则发布治理另留 deferred |
