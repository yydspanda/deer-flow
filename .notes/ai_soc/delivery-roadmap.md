...
| ID | Work / 工作 | Deliverable / 产出 | Acceptance / 验收 |
|---|---|---|---|
| `PI-01` | Real providers / 真实能力源 | **In Progress**: 已完成 212 条唯一 Zeus 告警的可追溯 corpus intake、PingAn source/parser Checkpoint B、structured fallback 修复和 95 条 NIDS Checkpoint C；下一步审阅 EDR/TI/SIEM field coverage，再选择第一项获批只读 dev/staging 集成 | corpus 保持唯一 ID、lineage 和显式敏感证据模式；10 条可用 structured fallback 均产生 bounded evidence，2 条空 rawLogs 显式报数据缺口；212/212 条、8/8 topic 经过统一生产 LLM 投影，112 条压缩 210 段且 raw hash 零变化；NIDS 95/95 五元组、128 network observations、67 HTTP observations 可回放；Adapter field-use review 通过；真实 provider contract、超时、权限、数据边界、审计、失败降级和 smoke 有证据 |
...
Next:         Review EDR field use after completed NIDS Checkpoint C, then TI/SIEM and the first approved endpoint
