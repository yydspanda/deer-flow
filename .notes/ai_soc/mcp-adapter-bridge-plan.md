...
> 状态：Phase 1 后续规划。目标是把真实资产系统、EDR 只读查询、F5/SOAR/MCP 能力接进 SOC action adapter registry，但不让 SOC Lead Agent 直接调用任意 MCP tool。
...
1. **MCP tool provider port + fake provider adapter tests**
   - 不接真实 MCP。
   - 固定 provider port、tool descriptor、timeout/error mapping、result payload。

2. **MCP-backed read-only `asset.lookup` adapter behind explicit config**
...
5. **High-risk MCP preflight only**
...
建议做 **MCP tool provider port + fake provider adapter tests**：

- 新增 `backend/soc_agent/mcp_adapters.py` 或 `backend/soc_agent/action_adapters_mcp.py`。
- 新增 `SocMcpToolProviderPort` 和 `FakeSocMcpToolProvider` 测试。
- 新增 `SocMcpToolActionAdapter` skeleton，只支持 read-only fake invocation。
- 不修改 DeerFlow MCP core，不接真实 MCP server。
