...
6. **运行环境与 Provider 目标分离**：generic Runtime 的 `dev|stg` 不能自行猜测远端系统环境；每个租户 Integration 必须显式声明 target environment、URL、host allowlist、凭证和高风险环境确认。PingAn Host deployment profile 在 Integration 边界受治理地绑定 `项目 DEV -> ZEUS PRD + Agent Platform PRD`、`项目 STG -> ZEUS STG + Agent Platform STG`，一次切换作用域和两组对应凭证；generic Runtime 不读取这些 PingAn 变量。
...
- Memory 库存修订调用 `SocMemoryService.propose_revision_candidate()` / `POST /api/soc/memory/records/{memory_id}/revision-candidates`，并显式区分 `observed_use` 与 `operator_direct`。前者必须携带实际 `source_run_id` 并核验持久化 `SocMemoryUseRecord` 及使用时 content/facet hash；后者冻结当前 predecessor version/hash，不伪造 use。两者携带 `expected_record_version`、typed issue、理由、可信 actor 和幂等键，在一个 mutation transaction 中 CAS 暂停旧 retrieval、创建带 immutable lineage 的 pending candidate 并写 audit。已有候选的审核可显式提交 `replaces_memory_id + expected_replaced_version + reviewed lesson/verdict`，在确认事务内连接同一 revision/supersession 机制；选择本身不写库，不增加必填理由表单。不得由 React、router 或 repository 直接覆盖旧 Lesson，同一 Memory 同时只允许一个 open revision。
- 启用经验前按完整有效 inventory 检查 typed applicability，不能用 retrieval Top-K 代替。相同强行为范围的重复答案应补充/修订；相同范围相反结论（含 context-only）与重叠范围相反 directive 不得同时启用。Profile 声明的单值 facet 才可用于证明范围互斥，实体列表不同不是范围互斥。SQLite 写事务、PostgreSQL transaction advisory lock 与 CAS 同时保护检查/写入；相反新结论必须由审核人确认，不能由模型输出或企业转交自动停用旧经验。旧重复数据不自动清理，Runtime 多指令冲突仍兜底。
...
- 检索命中不等于最终模型输入。`ConfirmedMemoryAnalysisRequestEnricher` 在冻结目录前对有界候选应用
  `soc.memory_context_precedence.v1`：同 canonical 检测/场景内，已精确匹配行为指纹且审核结论明确的经验，
  优先于结论相反的 partial 经验。后者只进入 `LLMAnalysisRequest.memory_context_exclusions` 差异审计，
  保留版本/hash/匹配差异/优先经验，不分配可引用 M-*，不进入 MemoryUse 或使用统计，不被投影为 evidence gap。
  未命中精确经验、精确结论不明确/相互矛盾、不同检测范围时不得据此排除；不改变库存激活或原始检索结果。
...
- `SocAgentActionPolicy` 是 action 执行前权限闸门：
  ...
  - Runtime 环境与 Provider target 在底层契约上必须正交，generic Runtime 不得推导 PingAn endpoint；但 PingAn Host 部署 profile 必须执行已批准映射 `DEV -> ZEUS PRD + Agent Platform PRD`、`STG -> ZEUS STG + Agent Platform STG`。mode-`0600` 私有 env 保存 active target 和环境 profile；受治理切换命令一次性更新 `SOC_PINGAN_ENV`、active ZEUS endpoint/allowlist/credential，以及 active Agent Platform endpoint/allowlist/app identity/三个资产归属 workflow ID。Host 启动、preflight 和 internal Asset Provider 构造均对错配 fail closed。旧源码只证明 Agent Platform STG endpoint，不包含 STG `YHSYS` identity/workflow IDs；不得用 `jtxxaq` 等无关身份替代，资料补齐前项目 STG 切换必须在写文件前失败。模型 target、lifecycle/callback Provider mode 和 action authority 不随该命令改变；状态只展示 Runtime、数据库、target environment 和映射是否一致，不展示凭证。
...
PingAn 完整交付的源码冻结必须先通过固定旧版本兼容门禁：从已交付 commit 的代码
生成并保存合成 Run/Observation/审核经验，普通测试不得用当前代码重新生成预期。
临时 SQLite 验证旧记录列表、执行、固定审计与持久化原文不变，以及新版查询复用
旧经验的正例和条件、租户、环境、版本、治理限制的反例。打包器强制执行指定测试；
失败、跳过、缺依赖或测试、零执行、超时均阻断，无跳过开关。通过回执绑定源码
commit、完整文件指纹、基线及源码归档哈希；检查或打包期间源码变化须重新检查。
验证及验包完成前不替换上一份交付物；该门禁不调用模型、不访问业务库，不增加
内网 Mac 部署步骤，也不得以重跑第一批或重新审核来掩盖不兼容。
