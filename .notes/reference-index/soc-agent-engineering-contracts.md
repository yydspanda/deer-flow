...
6. **运行环境与 Provider 目标分离**：generic Runtime 的 `dev|stg` 不能自行猜测远端系统环境；每个租户 Integration 必须显式声明 target environment、URL、host allowlist、凭证和高风险环境确认。PingAn Host deployment profile 在 Integration 边界受治理地绑定 `项目 DEV -> ZEUS PRD`、`项目 STG -> ZEUS STG`，一次切换作用域和对应凭证；generic Runtime 不读取这些 PingAn 变量。
...
- `SocAgentActionPolicy` 是 action 执行前权限闸门：
  ...
  - Runtime 环境与 Provider target 在底层契约上必须正交，generic Runtime 不得推导 PingAn endpoint；但 PingAn Host 部署 profile 必须执行已批准映射 `DEV -> ZEUS PRD`、`STG -> ZEUS STG`。mode-`0600` 私有 env 同时保存 PRD/STG 两套 profile，受治理切换命令一次性更新 `SOC_PINGAN_ENV` 与 active ZEUS endpoint/allowlist/credential，Host 启动和 preflight 对错配 fail closed。模型/Agent Platform target、lifecycle/callback Provider mode 和 action authority 不随该命令改变；状态只展示 Runtime、数据库、target environment 和映射是否一致，不展示凭证。
