...
def build_evidence_grounding_review(
    ...
    *,
    ...
    analysis_hash_before = canonical_sha256(analysis.model_dump(mode="json", exclude_none=True))
    grounding = ground_analysis_evidence(analysis, request)
    analysis_hash_after = canonical_sha256(analysis.model_dump(mode="json", exclude_none=True))

    checks = {
        "d7_acceptance_allows_continuation": (d7_acceptance.get("status") in _ALLOWED_D7_STATUSES),
        "d5_alert_id_matches": (str(_mapping_path(skill_context_review, "input", "alert_id")) == str(alert_id)),
        "d7_alert_id_matches": (str(_mapping_path(analyzer_output_review, "input", "alert_id")) == str(alert_id)),
        "d7_links_exact_d5_request": (d7_input.get("d5_request_sha256") == request_hash),
        "d7_analysis_hash_matches": (analyzer_output_review.get("analysis_result_sha256") == analysis_hash_before),
        "grounding_did_not_mutate_analysis": (analysis_hash_before == analysis_hash_after),
        "grounding_covers_every_evidence_item": (grounding.total_count == len(analysis.evidence) and len(grounding.items) == len(analysis.evidence)),
        "grounding_counts_are_consistent": (grounding.grounded_count + grounding.ungrounded_count == grounding.total_count),
        "description_leakage_is_fail_closed": all(item.status is not AnalysisEvidenceGroundingStatus.GROUNDED for item in grounding.items if item.foreign_description_context_paths),
    }
    ...
    if grounding.description_leakage_count:
        blocking_reasons.append("description_context_leakage")
    if any("outcome-success claim" in warning for warning in grounding.warnings):
        blocking_reasons.append("unproven_outcome_claim")

    grounding_by_index = {item.evidence_index: item for item in grounding.items}
    scenario_support = []
    for scenario in analysis.scenario_assessments:
        referenced_items = [grounding_by_index[index] for index in scenario.evidence_indices]
        rejected_indices = [item.evidence_index for item in referenced_items if item.status is not AnalysisEvidenceGroundingStatus.GROUNDED]
        scenario_support.append(
            {
                ...
                "evidence_indices": scenario.evidence_indices,
                "grounded_evidence_indices": [item.evidence_index for item in referenced_items if item.status is AnalysisEvidenceGroundingStatus.GROUNDED],
                "rejected_evidence_indices": rejected_indices,
                "all_referenced_evidence_grounded": not rejected_indices,
            }
        )

    evidence_review = []
    for evidence_index, evidence in enumerate(analysis.evidence):
        grounding_item = grounding_by_index[evidence_index]
    ...
    return {
        ...
        "scope": {
            "performed": [
                ...
                "source_and_value_matching",
                "description_sibling_fact_leakage_audit",
                "scenario_evidence_support_projection",
            ],
            "not_performed": [
                "llm_call",
                "decision_policy",
...
def main() -> int:
    ...
    analyzer_output_review_path = args.analyzer_output_review or (DEFAULT_CHECKPOINT_D_ROOT / "step-d7-analyzer-output" / f"{args.alert_id}.analyzer-output.json")
    output_dir = args.output_dir or (DEFAULT_CHECKPOINT_D_ROOT / "step-d8-evidence-grounding")
    skill_context_review = json.loads(skill_context_review_path.read_text(encoding="utf-8"))
    analyzer_output_review = json.loads(analyzer_output_review_path.read_text(encoding="utf-8"))
        ...
        json.dumps(
            {
                ...
                "description_leakage_count": review["grounding_report"]["description_leakage_count"],
