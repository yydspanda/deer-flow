...
def test_evaluate_shadow_batches_passes_real_paired_five_row_gate(tmp_path: Path) -> None:
...
def test_evaluate_shadow_batches_detects_pre_llm_projection_drift(
    ...
    assert report["metrics"]["paired_compatibility"]["deterministic_projection_mismatch_count"] == 1
...
def _build_case(tmp_path: Path, *, route: str = "asset.locate") -> dict[str, Path]:
    ...
    adapter_id = "asset-locate-pingan-mcp" if route == "asset.locate" else "asset-lookup-pingan-mcp"
    composition = {
