...
@pytest.mark.parametrize(
    "contender",
    ["create", "get_or_create", "bind_metadata_once", "claim_unowned", "update_display_name", "update_status", "update_metadata", "update_owner", "delete"],
)
async def test_all_memory_mutations_share_the_per_thread_lock(contender):
    ...
    operations = {
        ...
        "get_or_create": lambda: store.get_or_create("locked-thread", user_id=None),
        "bind_metadata_once": lambda: store.bind_metadata_once("locked-thread", "binding", "first", user_id=None),
...
@pytest.mark.anyio
@pytest.mark.no_auto_user
async def test_get_or_create_changes_incarnation_only_after_delete(store):
    first = await store.get_or_create("recreated-thread", user_id="owner")
    reused = await store.get_or_create("recreated-thread", user_id="owner")
    await store.delete("recreated-thread", user_id="owner")
    recreated = await store.get_or_create("recreated-thread", user_id="owner")

    assert reused["incarnation"] == first["incarnation"]
    assert recreated["incarnation"] != first["incarnation"]
    assert not store._thread_locks._entries_by_loop
