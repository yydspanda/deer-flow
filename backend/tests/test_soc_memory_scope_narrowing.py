...
def test_selected_values_narrow_scope_and_context_without_mutating_candidate():
    original = scope()
    result = promote_memory_applicability_facets(original, ["source_type"], {"entity": ["ip:10.0.0.1"]})
    assert result.required_facets["entity"] == ["ip:10.0.0.1"]
    assert result.required_facets["source_type"] == ["ndr"]
    assert "entity" not in result.optional_facets
    assert "entity" in result.context_only_required_facet_keys
...
def test_optional_threshold_cannot_be_weakened_by_promotion():
    ...
    with pytest.raises(ValueError, match="too few optional"):
        promote_memory_applicability_facets(original, [], {"entity": ["ip:10.0.0.1"]})


@pytest.mark.parametrize("fingerprint", ["hash", "other-behavior"])
def test_selected_entity_blocks_both_exact_and_context_use_when_only_old_rule_matches(fingerprint):
    ...
    assert "entity" in report.missing_required_facet_keys
