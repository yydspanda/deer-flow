...
def test_load_legacy_model_gateway_profile_resolves_named_literals(tmp_path: Path) -> None:
    model_source, root_source, _key_hex = _legacy_sources(tmp_path)
...
def test_prepare_legacy_model_gateway_env_writes_private_files_without_secret_report(
    ...
    model_source, root_source, key_hex = _legacy_sources(tmp_path)
...
def _legacy_sources(tmp_path: Path) -> tuple[Path, Path, str]:
    ...
    return model_source, root_source, key_hex
