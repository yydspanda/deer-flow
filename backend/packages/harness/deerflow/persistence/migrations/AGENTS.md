...
The in-memory thread store's atomic `get_or_create` and `bind_metadata_once`
operations share the same per-thread keyed lock as ordinary mutations. Newly
created records receive an incarnation; reuse preserves it and delete/recreate
changes it. Do not add a separate metadata lock that bypasses this lifecycle.
