"""Read-only stdio MCP wrapper for PingAn ZEUS security tags."""
...
_SERVER_NAME = "soc-pingan-security-tag"
_SERVER_VERSION = "0.1.0"
_DEFAULT_PROTOCOL_VERSION = "2025-11-25"
_TOOL_NAME = "security_tag_lookup"
...
_INPUT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        "entity_key": {
            "type": "string",
            "description": "Exact entity value selected by the governed SOC investigation planner.",
        },
        "entity_type": {
            "type": ["string", "null"],
            "description": "Optional vendor-neutral entity type such as ip, host, domain, or account.",
        },
    },
...
_OUTPUT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "provider_records_found": {"type": "boolean"},
        "security_tag_found": {"type": "boolean"},
        "has_active": {"type": "boolean"},
        "lookup_status": {
            "type": "string",
            "enum": [
                "active",
        ...
        "security_tag": {"type": ["object", "null"]},
        "records": {"type": "array", "items": {"type": "object"}},
        "provider_record_count": {"type": "integer", "minimum": 0},
        "records_omitted_count": {"type": "integer", "minimum": 0},
        "provider_version": {"type": ["string", "null"]},
        "response_sha256": {"type": "string"},
        "source_freshness": {"type": "string", "const": "unknown"},
        ...
        "authorization_fact_created": {"type": "boolean", "const": False},
...
def main() -> None:
    try:
        service = build_pingan_security_tag_service_from_env()
    ...
    for line in sys.stdin:
        ...
        except Exception as exc:  # noqa: BLE001 - keep protocol errors structured
    ...
    *,
    ...
    if method == "initialize":
        ...
        return _success_response(
            request_id,
            {
                ...
                "instructions": "Read-only PingAn ZEUS security tags. A match is investigation evidence, not authorization.",
            },
        )
        ...
        return _success_response(
            request_id,
            {
                "tools": [
                    {
                        "name": _TOOL_NAME,
                        "description": "Query bounded PingAn ZEUS tag records without changing verdict, disposition, or authorization facts.",
    ...
    if method == "tools/call":
        if startup_error or service is None:
            return _success_response(request_id, _tool_error(f"PingAn security-tag provider unavailable: {startup_error}"))
        params = message.get("params") if isinstance(message.get("params"), dict) else {}
        if params.get("name") != _TOOL_NAME:
            return _success_response(request_id, _tool_error(f"unknown PingAn security-tag tool: {params.get('name')}"))
        arguments = params.get("arguments") if isinstance(params.get("arguments"), dict) else {}
        try:
            query = PingAnSecurityTagQuery.model_validate(arguments)
            result = service.lookup(query).model_dump(mode="json", exclude_none=False)
        except (ValidationError, PingAnSecurityTagProviderError) as exc:
...
def _safe_error(exc: Exception) -> str:
    if isinstance(exc, PingAnSecurityTagProviderError):
